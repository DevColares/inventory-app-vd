          ...
          <table className="min-w-full divide-y divide-slate-100 dark:divide-slate-800 session-table">
            ...
            <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-50 dark:divide-slate-800">
              {filteredItems.map((item, index) => {
                return (
                  <tr key={item.ean + item.timestamp} className={`hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors ${index === 0 ? 'bg-blue-50/20 dark:bg-blue-900/10' : ''}`}>
                    <td className="px-4 py-4">
                      <span className="block font-bold text-slate-900 dark:text-slate-100 text-sm truncate product-name-cell">{item.name}</span>
